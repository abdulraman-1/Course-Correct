      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className={`${statusColors[status]} px-3 py-1 rounded-full text-sm font-medium`}>
            {status === "completed" && <FontAwesomeIcon icon={faCheckCircle} className="mr-2" />}
            {status.replace(/^\w/, c => c.toUpperCase())}
          </div>
          
          <div>
            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{schedulelesson}</p>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mt-1">{scheduletitle}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{scheduledate}</p>
          </div>
        </div>
